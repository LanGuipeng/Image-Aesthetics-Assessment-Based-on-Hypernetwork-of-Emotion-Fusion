...
logging = get_logger('/myDockerShare/zys/aes-emotion/experiment/ablation.txt')
...
def get_dataloaders(
    ...
    train_ds = AVADataset(os.path.join(path_to_save_csv , "train.csv"), '/myDockerShare/zys/ava/aes-im', transform.train_transform)
    val_ds = AVADataset(os.path.join(path_to_save_csv , "test.csv"), '/myDockerShare/zys/ava/aes-im', transform.val_transform)
    #test_ds = AVADataset(os.path.join(path_to_save_csv , "try.csv"), '/myDockerShare/zys/ava/aes-im', transform.val_transform)
...
class Trainer:
    def __init__(
        ...
        *,
        ...
        self.model_hyper = models3.HyperNet(16, 112, 224, 112, 56, 28, 14, 7).cuda()
    ...
    def train(self):
        ...
        for t in range(0, self.num_epoch ):
            ...
            for idx, (x, y) in enumerate(self.train_loader):
                ...
                x = x.to(self.device)
                y = y.to(self.device)
                paras = self.model_hyper(x)
                ##参数量
                # flops, params = profile(self.model_hyper, inputs=(x,))
                # # print('FLOPs = ' + str(flops/1000**3) + 'G')
                # print('Params = ' + str(params/1000**2) + 'M')
                model = models3.TargetNet(paras).cuda()
                model.train()
                ...
                if idx % self.print_freq:
                    ...
                    print(f"iter #[{idx}/{total_iter}] " f"loss = {loss:.3f} " f"time = {log_time:.2f} " f"eta = {eta:.2f}" f"time1={t:.4f}")
            ...
            if val_srcc > best_srcc:
                ...
                torch.save(best_state, os.path.join(self.experiment_dir ,"best_state3.pth"))
